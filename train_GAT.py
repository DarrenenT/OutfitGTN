...
from models.OutfitGAT import OutfitGAT
...
def main():
    # Load config
    with open('config/config_GAT.yaml', 'r') as f:
    ...
    logger.info(f"Initialized OutfitGAT model with {sum(p.numel() for p in model.parameters())} parameters")
    ...
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    writer = SummaryWriter(log_dir=f"{config['training']['log_dir']}/run_{timestamp}")
    
    # Check if we should resume from checkpoint
    start_epoch = 0
    checkpoint_path = f"{config['training']['checkpoint_dir']}/best_model.pt"
    resume_path = f"{config['training']['checkpoint_dir']}/latest_checkpoint.pt"
    ...
    for epoch in range(start_epoch, config['training']['num_epochs']):
        ...
        if val_metrics['val_loss'] < best_val_loss:
            ...
            trainer.save_checkpoint(
                path=f"{config['training']['checkpoint_dir']}/best_model.pt",
    ...
    logger.info(f"Final best model saved at: {config['training']['checkpoint_dir']}/best_model.pt")

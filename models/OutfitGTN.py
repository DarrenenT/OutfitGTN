...
class OutfitGTN(nn.Module):
    def __init__(
        ...
        # TransformerConv layers instead of GATConv
        self.conv_1 = TransformerConv(
            ...
            root_weight=True  # Apply transformation to root node
        )

        self.conv_2 = TransformerConv(
        ...
        # LayerNorm
        self.layer_norm_1 = nn.LayerNorm(hidden_channels[1])
        self.layer_norm_2 = nn.LayerNorm(hidden_channels[1])
        ...
        # Hop attention weights (for combining 1-hop and 2-hop)
    ...
    def _process_single_graph(self, data: Data) -> torch.Tensor:
        """Process a single graph with transformer-based message passing"""
        ...
        # TransformerConv message passing with residual connections
        one_hop = self.conv_1(out, edge_index)  # [num_nodes, hidden_channels[1]]
        one_hop = self.layer_norm_1(one_hop)
        one_hop = F.relu(one_hop)
        
        two_hop = self.conv_2(one_hop, edge_index)  # [num_nodes, hidden_channels[1]]
        two_hop = self.layer_norm_2(two_hop)
        two_hop = F.relu(two_hop)
        
        # Weighted combination of 1-hop and 2-hop representations
        out = self.hop_attention[0] * one_hop + self.hop_attention[1] * two_hop
        
        # Feature gating
        gates = self.feature_gate(out)
        out = out * gates
        
        # Add residual connection from first layer
        if self.residual:
